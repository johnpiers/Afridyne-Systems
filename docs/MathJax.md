...
![](imgs/20260105-112016.png){ .center-image }
...
!!! abstract "LaTeX/Package"
    ...
    The package ${{LaTeX/Parameter|dvipsnames}}$ allows you to access more than 60 colors, and the package ${{LaTeX/Parameter|svgnames}}$ allows access to about 150 colors. If you need more color names, then you may also want to look at the ${{LaTeX/Parameter|x11names}}$ option that offers more than 300 colors.
...
!!! pied-piper "When there are 2 Solutions"
...
!!! desc "The Riemann Curvature Tensor"

    **What it is:** The central mathematical object in differential geometry that measures the **intrinsic curvature** of a space.
    
    **Why it matters:** It tells you how much a manifold bends or twists. In Einstein's General Relativity, this tensor is what dictates how mass and energy warp spacetime, effectively defining gravity.
    
    $\begin{aligned}\displaystyle R^\rho_{\ \sigma\mu\nu} = \partial_\mu \Gamma^\rho_{\ \nu\sigma} - \partial_\nu \Gamma^\rho_{\ \mu\sigma} + \Gamma^\rho_{\ \mu\lambda}\Gamma^\lambda_{\ \nu\sigma} - \Gamma^\rho_{\ \nu\lambda}\Gamma^\lambda_{\ \mu\sigma}\end{aligned}$


!!! quote "Geometric Interpretation"

    The Riemann tensor can also be compactly defined by analyzing how a vector field $X$ changes when you parallel transport it in a loop defined by two other vector fields, $Y$ and $Z$:

    $\begin{aligned}R(Y, Z)X = \nabla_Y \nabla_Z X - \nabla_Z \nabla_Y X - \nabla_{[Y, Z]} X\end{aligned}$
...
!!! quote "The Lorenz Equations"
    ...
    The Lorenz equations are a system of three ordinary differential equations (ODEs) originally derived by meteorologist and mathematician Edward N. Lorenz in 1963. They model atmospheric convection, but became famous for being one of the first mathematical systems to demonstrate deterministic chaos and the "butterfly effect".
...
!!! quote "A Rogers-Ramanujan Identity"
    ...
    A Rogers-Ramanujan Identity: $1 + \frac{q^2}{(1-q)} + \frac{q^6}{(1-q)(1-q^2)} + \dots = \prod_{j=0}^{\infty} \frac{1}{(1-q^{5j+2})(1-q^{5j+3})}, \quad\quad \text{for } |q| < 1$

!!! quote "A Rogers-Ramanujan Identity"

    A Rogers-Ramanujan Identity:

    $1 + \frac{q^2}{(1-q)} + \frac{q^6}{(1-q)(1-q^2)} + \dots = \prod_{j=0}^{\infty} \frac{1}{(1-q^{5j+2})(1-q^{5j+3})}, \quad\quad \text{for } |q| < 1$
...
!!! quote "Maxwell's Equations"
    ...
    #### Maxwell's Equations {.toc-hidden-header}
    
    Maxwell's Equations:
    
    (Single Block: Code left-aligned)
    ...
    &\nabla \times \mathbf{B} = \mu_0 \left( \mathbf{J} + \varepsilon_0 \frac{\partial \mathbf{E}}{\partial t} \right) && \text{(Ampère-Maxwell's Law)}
    \end{aligned}$
    
    ---
    
    (Individual: Code left-aligned) 
    ...
    \(\nabla \times \mathbf{B} = \mu_0 \left( \mathbf{J} + \varepsilon_0 \frac{\partial \mathbf{E}}{\partial t} \right) \quad \text{(Ampère-Maxwell's Law)}\)
    
    ---
    
    (Inline variation below, of the above code:)
...
!!! example "2. Plugging into the Formula"
    ...
    Simplifying that, we get: $1 - \frac{x^2}{2!} + \frac{x^4}{4!} - \frac{x^6}{6!} + \dots$
...
!!! success "3. Generalizing with Sigma Notation"
    ...
    This leads us exactly to:  $\cos x = \sum_{k=0}^{\infty} \frac{(-1)^k}{(2k)!} x^{2k}$
...
!!! quote "The Maclaurin Expansion of $\cos(x)$"
    ...
    As you add more terms from the series, the approximation "hugs" the actual cosine wave further and further away from zero, which is mathematically expressed as $\displaystyle \cos x = 1 - \frac{x^2}{2!} + \frac{x^4}{4!} - \frac{x^6}{6!} + \dots$
...
??? desc "`MathJax .js config` `Click To See Code` "
...
??? desc "`yaml restructure` `Click To See Code`"
...
!!! desc "Rogers-Ramanujan Identity 2"
    ...
    The given expression is the standard form of the **Second Rogers-Ramanujan Identity**, which equates an infinite series to an infinite product.
    
    ---
    
    #### 2. Verify the Series Side
    
    The left-hand side (LHS) is expressed as the summation $\sum_{n=0}^{\infty} \frac{q^{n(n+1)}}{(q; q)_n}$, where $(q; q)_n = \prod_{k=1}^n (1-q^k)$ is the $q$-Pochhammer symbol, yielding terms like $1 + \frac{q^2}{1-q} + \frac{q^6}{(1-q)(1-q^2)} + \cdots$ for $n=0,1,2$.
    
    ---
    
    #### 3. Verify the Product Side
...
The given expression represents the component expansion of the **covariant derivative** of a vector field $Y$ along a vector field $X$. This formula uniquely defines the **Christoffel Symbols of the Second Kind** ($\Gamma_{im}^k$) as the connection coefficients of the Levi-Civita connection.

### 2. Verify the Directional Derivative

The expression transitions from an abstract coordinate-free notation to a localized coordinate frame:
* $(\nabla_X Y)^k$ represents the $k$-th component of the covariant derivative along the full vector field $X$.
* By utilizing the linearity of the connection, we extract the component fields $X^i$, rewriting the operator as $X^i (\nabla_i Y)^k$. This isolates the derivative with respect to the coordinate basis vector $\partial_i$.

### 3. Verify the Index Expansion

Evaluating the localized operator $(\nabla_i Y)^k$ reveals how the vector field changes, accounting for both the changing components and the changing basis vectors of a curved manifold or curvilinear system:
* **The Ordinary Derivative**: $\frac{\partial Y^k}{\partial x^i}$ captures the standard calculus rate of change of the component $Y^k$ along the $x^i$ direction.
* **The Correction Term**: $\Gamma_{im}^k Y^m$ acts as a geometric correction. It uses the Einstein summation convention over the dummy index $m$ to compute how the underlying basis vectors twist, stretch, or rotate across the space.


!!! deep-dive "Definition of Christoffel Symbols"

    $(\nabla_X Y)^k = X^i (\nabla_i Y)^k =
          X^i \left( \frac{\partial Y^k}{\partial x^i} + \Gamma_{im}^k Y^m \right)$
    
!!! deep-dive "Curl of a Vector Field"
...
!!! deep-dive "Gauss' Divergence Theorem"

    $\int_D ({\nabla\cdot} F)dV=\int_{\partial D} F\cdot ndS$
    
!!! tldr "Angle Sum Formula for Cosines"

    $\cos(\theta+\phi)=\cos(\theta)\cos(\phi)−\sin(\theta)\sin(\phi)$
    
!!! tldr "Cauchy's Integral Formula"

    $f(a) = \frac{1}{2\pi i} \oint\frac{f(z)}{z-a}dz$
    
!!! tldr "The Quadratic Formula"

    $x = {-b \pm \sqrt{b^2-4ac} \over 2a}$
    
### The Fourier Transform

!!! desc "The Fourier Transform"

    **What it is:** A mathematical tool that decomposes a function or signal into its constituent **frequencies**.
    
    **Why it matters:** It bridges the gap between the time domain and the frequency domain. It underpins modern signal processing, JPEG/MP3 compression, medical imaging (MRI), and audio engineering.
    
    $\hat{f}(\xi) = \int_{-\infty}^{\infty} f(t) e^{-2\pi i t \xi} \, dt$

!!! quote "The Inverse Fourier Transform"
    To reverse the process and reconstruct the original time-domain signal from its frequency components, the inverse transform is used:
