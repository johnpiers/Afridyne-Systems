...
![](imgs/20260222-132325.png){: style="display: block; margin: 0 auto"}
<H1 style="text-align: center;"> Getting Started</H1>

![](imgs/20251230-175207.png){ .center-image }
...
<div class="admonition danger"><p class="admonition-title">Warning</p><p>The caption can stretch over multiple lines. However it must end with a blank line before the image. Otherwise the image will be interpreted as part of the caption.</p></div>
...
=== "FreeBSD"
    <p>Binary packages can be installed using <code>pkg</code>:</p>
    ```
    $ pkg install mercurial
    ```
    <p>Alternatively, install from source via the ports collection:</p>
    ```
    $ cd /usr/ports/devel/mercurial
    $ make install
    ```

=== "Windows (Mercurial CLI)"
    <p>Using the <code>winget</code> package manager:</p>
    ```powershell
    PS> winget install Mercurial.Mercurial -e
    ```
    <p>Or download from the list of binary releases.</p>

=== "Windows (TortoiseHg)"
    <p>Using the <code>winget</code> package manager:</p>
    ```powershell
    PS> winget install TortoiseHg.TortoiseHg -e
    ```
    <p>Or download from the list of binary releases.</p>
</div>
