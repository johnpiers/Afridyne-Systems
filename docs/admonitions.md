...
??? warning "This configuration method is deprecated"
  ...
  [Admonitions]: MkDocs-Material/python-markdown.md#admonitions
...
!!! desc "Expanded Collapsable Blocks"
...
:simple-pythonanywhere: :japanese_castle: :jellyfish: :snake: :simple-python:  :party_popper:  :airplane_departure:  :lizard: :mouse: :spider: :material-spider-outline: :sponge:

## Markdown Title and {.no-copy }<a id="Markdown Title and {.no-copy }"></a><a id="Markdown Title and {.no-copy }"></a>

!!! desc "Custom Admonition"
    **Two custom admonitions of the same thing with a slight variation to the `markdown title and {.no-copy }` setting.**
    
!!! pied-piper "Pied Piper"
    ``` { .markdown title="rsync examples" .no-copy }
