...
![](../imgs/01-introduction.png){ .center-image }

<div style="display: none;"><h1>01-introduction</h1></div>

<h3 style="text-align: center;">Chapter 1: A short <i>introduction.</i></h3>

!!! desc "The Z-Shell"

    ## Chapter 1: A short introduction. {.toc-hidden-header}
...
!!! desc "The most basic basics:"

    ## The most basic basics: {.toc-hidden-header}
    ...
    You're also going to need zsh itself; if you're reading this, you may well already have it, but if you don't, you or your system administrator should read [Appendix A](08-appendix.md#appendix-1-obtaining-zsh-and-getting-more-information-not-written). For now, we'll suppose you're sitting in front of a terminal with zsh already running.
    
!!! desc "Now to the shell."

    ## Now to the shell! {.toc-hidden-header}
    ...
    - Customise the environment in which you work, by using startup files,- write your own commands to shorten tasks and store things in shell variables (`parameters`) so you don't have to remember them,- use zle to minimise the amount of typing you have to do --- in zsh, you can even edit small files that way,- pick the files you want to use for a particular command such as `mv` or `ls` using zsh's very sophisticated filename generation (known colloquially as `globbing`) system,- tell the editor what sort of arguments you use with particular commands, so that you only need to type part of the name and it will complete the rest, using zsh's unrivalled programmable completion system,- use the extra add-ons (`modules') supplied with the latest version of zsh to do other things you usually can't do in a shell at all.
...
!!! abstract "Things to know straight away!"

    ## Things to know straight away! {.toc-hidden-header}
...
!!! recommendation "Back-ground Basics"
...
!!! recommendation "Another Common Family of Shells"
...
!!! version-added "Versions of zsh"
    ...
    One notable feature of zsh is the completion of command line arguments. The system changed in 3.1.6 and 3.1.7 to make it a lot more configurable, and (provided you keep your wits about you) a little less obscure. I therefore won't describe the old completion system, which used the `compctl` command, in any detail; a very brief introduction is given in the zsh FAQ. The old system remains available, however we strongly recommend new users to start with the new one. See [chapter 6](06-completion.md#chapter-6-completion-old-and-new) `Completion, old and new' for the lowdown on new-style completion.
    
    There won't be a big difference between 4.0 and 4.1, just bug fixes and a few evolutionary changes, plus some extra modules. There will be some notes in [chapter 7](07-modules.md#chapter-7-modules-and-other-bits-and-pieces-not-written) about new features in 4.1, but nothing you write for 4.0 is likely to become obsolete in the foreseeable future.
    


## 1.3: Conventions

!!! recommendation "1.3: Conventions"
    ...
    but sometimes I will show both what to type into a shell interactively, and what the shell throws back at you:
