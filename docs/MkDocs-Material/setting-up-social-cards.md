...
:fontawesome-brands-youtube:{ style="color: #EE0F0F" } __[How to build custom social cards]__ by @james-willett – :octicons-clock-24: 24m –  Learn how to create entirely custom social cards perfectly matching your branding for each page automatically!
...
Social card of our [formatting] reference
  ...
  [default layouts]: social.md#layouts
...
!!! abstract " built-in social plugin"
...
For a list of all settings, please consult the [plugin documentation].
  ...
  [plugin documentation]: social.md

!!! info "The [`site_url`][site_url] setting must be set"

    Note that you must set [`site_url`][site_url] when using the social plugin, or the generated cards will not be correctly linked. Social media services like X and Facebook demand that social previews point to an absolute URL, which the plugin can only compute when [`site_url`][site_url] is set.
  ...
  [dependencies for image processing]: image-processing.md
  [site_url]: https://www.mkdocs.org/user-guide/configuration/#site_url

## Usage

- If you want to adjust the title or set a custom description for the social card:
-  You can set the front matter [`title`][`Changing the title`] and [`description`][`Changing the description`] properties, which take precedence over the defaults:
- Or use:

    - [`cards_layout_options.title`](social.md#option.title)
    - [`cards_layout_options.description`](social.md#option.description)

  
### Choosing a Font.

- Some fonts do not contain CJK characters, like for example the [default font, `Roboto`][font].
- In case your `site_name`, `site_description`, or:
- Page title contain CJK characters, choose another font from [Google Fonts] which comes with CJK characters, e.g. one from the `Noto Sans` font family:
...
!!! info ""
    ...
    If you want to use a different layout for a single page (e.g. your landing page), you can use the `social` front matter property together with the [`cards_layout`](social.md#meta.social.cards_layout) key, exactly as in `mkdocs.yml`:
...
!!! info ""
    ...
    Besides changing the entire layout, you can override all options that a layout exposes. This means you can parametrize social cards with custom front matter properties, such as `tags`, `date`, `author` or anything you can think of. Simply define [`cards_layout_options`](social.md#meta.social.cards_layout_options):
...
??? info "Code"
    ...
    ```markdown
    <div class="mdx-social" markdown>

    <div class="mdx-social__layer" markdown>
    <div class="mdx-social__image" markdown>
    <span class="mdx-social__label">Layer 0</span>
    ![Layer 0](../../assets/assets/screenshots/social-cards-layer-0.png)
    </div>
    </div>
    ...
    ![Layer 1](../../assets/assets/screenshots/social-cards-layer-1.png)
    </div>
    </div>
    ...
    ![Layer 2](../../assets/assets/screenshots/social-cards-layer-2.png)
    </div>
    </div>
    ...
    ![Layer 3](../../assets/assets/screenshots/social-cards-layer-3.png)
    </div>
    </div>
    ...
    ![Layer 4](../../assets/assets/screenshots/social-cards-layer-4.png)
    </div>
    </div>
    ...
    ![Layer 5](../../assets/assets/screenshots/social-cards-layer-5.png)
    </div>
    </div>
...
Layouts are written in YAML syntax. Before starting to create a custom layout, it is a good idea to [study the pre-designed layouts], in order to get a better understanding of how they work. Then, create a new layout and reference it in `mkdocs.yml`:



!!! info "Layout and Plugin Configuration"
...
- Note that the `.yml` file extension should be omitted. Next, run `mkdocs serve`, and see how the `.cache` directory is populated with the generated cards. Open any card in your editor, so you can see your changes immediately.

- Since we haven't defined any layers, the cards are transparent.
...
!!! quote ""
    ...
    layers:
      - size: { width: 1200, height: 630 }
        offset: { x: 0, y: 0 } # Aligned with 'size' above
    ```
    
- If the `size` is omitted, it defaults to the size of the layout. If the `offset` is omitted, it defaults to the top left corner, which is the default `origin`.

- Saving the layout and reloading renders:

![Layer size]

The layer outline and grid are visible because we enabled [`debug`][debug] mode in `mkdocs.yml`. The top left shows the layer index and offset, which is useful for alignment and composition.

  [Layer size]: ../assets/assets/screenshots/social-cards-layer-size.png
  [debug]: social.md#debugging
...
!!! info "Layer Alignment with Origin"
...
<H3 style="text-align: center;">The following table shows the supported values:</H3>
<figure markdown>
...
[Layer background color]: ../assets/assets/screenshots/social-cards-layer-background-color.png
[Layer background image]: ../assets/assets/screenshots/social-cards-layer-background-image.png
[Layer background]: ../assets/assets/screenshots/social-cards-layer-background.png

### Typography

!!! info "Dynamic Typography"
...
![Layer typography]
  ...
  [Layer typography]: ../assets/assets/screenshots/social-cards-layer-typography.png
...
!!! info "Truncating"
  ...
  [Layer typography ellipsis]: ../assets/assets/screenshots/social-cards-layer-typography-ellipsis.png
  [Layer typography shrink]: ../assets/assets/screenshots/social-cards-layer-typography-shrink.png
...
![Layer typography align]
  ...
  [Layer typography align]: ../assets/assets/screenshots/social-cards-layer-typography-align.png
...
!!! info "Fonts"
    ...
    In that case, the author can use `Bold` or `Medium Italic` as the `style`. If the font style specified in the layer is not part of the font family, the font always falls back to `Regular` and prints a warning in [`debug`][debug] mode, as `Regular` is included with all font families.
...
Note that this example makes use of [YAML anchors] to minify repetition. The `definitions` property is solely intended for the definition on aliases that can then be referenced with anchors.
...
__Are you missing something? Please [open a discussion] and let us know!__
  ...
  [Layer icon]: ../assets/assets/screenshots/social-cards-layer-icon.png
