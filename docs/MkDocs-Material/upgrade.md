...
<H1 style="text-align: center;">How to Upgrade</H1>
...
??? question "What's new? Click to Open!"
...
!!! desc "Changes to `*.html` files"
    ...
    - If you've overridden a __template__, check the respective `*.html` file for potential changes
...
!!! desc "`theme.features`"
  ...
  [tabs]:setting-up-navigation.md#navigation-tabs
...
??? desc "What's new? `Click to Open`"
  ...
  [CSS variables]: changing-the-colors.md#custom-colors
  [icon integration]: icons-emojis.md#search
  [prebuilt search indexes]: search.md
...
!!! desc "`theme.logo.icon`"

    - ### `theme.logo.icon`
    
    - The logo icon configuration was centralized under `theme.icon.logo` and can now be set to any of the [icons bundled with the theme][icon integration]:
...
!!! important "Changes to `mkdocs.yml`"
    None.
...
!!! important "Changes to `*.html` files"
