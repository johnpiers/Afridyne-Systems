...
!!! info "Cookie Consent"
    ...
    1. 🙋‍♂️ You can add arbitrary HTML tags in the `description`, e.g. to
    link to your terms of service or other parts of the site.
    
    * The following properties are available:
    
    1.1 This property sets the title of the cookie consent, which is rendered at the top of the form and must be set to a non-empty string.
    
    1.2 This property sets the description of the cookie consent, is rendered below the title, and may include raw HTML (e.g. a links to the terms of service).
    
    1.3 This property allows to add custom cookies or change the initial `checked` state and name of built-in cookies. Currently, the following cookies are built-in:
    
    - __Google Analytics__ – `analytics` (enabled by default)
    - __GitHub__ – `github` (enabled by default)
    
    1.4 Each cookie must receive a unique identifier which is used as a key in the `cookies` map, and can be either set to a string, or to a map defining`name` and `checked` state:
    ...
    ===  "Custom cookie"
        ...
        1.  🙋‍♂️ If you define a custom cookie as part of the `cookies`
                    property, the `analytics` cookie must be added back
                    explicitly, or analytics won't be triggered.
...
!!! abstract "Analytics"
    ...
    1. 🙋‍♂️ If the `manage` settings button is omitted from the
    `actions` property, the settings are always shown.
...
[![Cookie consent enabled]][Cookie consent enabled]
  ...
  [Cookie consent enabled]: ../assets/assets/screenshots/consent.png

#### Change Cookie Settings

!!! tip "Change Cookie Settings"
...
!!! pied-piper "Built-in Privacy Plugin"
  ...
  [plugin documentation]: privacy.md
...
!!! question "Why can't Material for MkDocs bundle all assets by design?"
  ...
  [example]: https://squidfunk.github.io/mkdocs-material/setup/ensuring-data-privacy/#example
  [built-in optimize plugin]: optimize.md
  [technical limitations]: privacy.md#limitations
...
!!! example "Custom Cookies"
    ...
    If you've customized the [cookie consent] and added a `custom` cookie, the user will be prompted to accept or reject your custom cookie. Once the user accepts or rejects the cookie consent, or [changes the settings], the page reloads[^1].
    
    Use [additional JavaScript] to query the result:
    
    [^1]:
        We reload the page to make interop with custom cookies simpler. If Material for MkDocs would implement a callback-
        based approach, the author would need to make sure to correctly update all scripts that use cookies. Additionally, the
        cookie consent is only answered initially, which is why we consider this to be a good trade-off of DX and UX.
