...
!!! desc "Hiding the Feedback Widget"
...
!!! desc "Custom Site Analytics"
    ...
    === ":octicons-file-code-16: `overrides/partials/integrations/analytics/custom.html`"
        ...
        1.  As an example, this variable receives the value set in
        `mkdocs.yml`, which is `"foobar"` for `property`.
        2.  If you're using [instant loading], you can use the
        location$` observable to listen for navigation events,
        which always emits the current `URL`.
...
!!! desc "Custom Site Feedback"
    ...
    === ":octicons-file-code-16: `docs/javascripts/feedback.js`"
        ...
        5.  Show the configured notes. Which one is shown depends on
        the user feedback.
