...
!!! example "Adding Assets Plus Additional CSS "
...
!!! example " Extending The Theme "
    ...
    1. If you want to alter the HTML source (e.g. add or remove some parts), you can extend the theme. MkDocs supports [theme extension], an easy way to override parts of Material for MkDocs without forking from git. This ensures that you can update to the latest version more easily.
    
    2. Setup and Theme Structure.
    
    3. Enable Material for MkDocs as usual in `mkdocs.yml`, and create a new folder for `overrides` which you then reference using the [`custom_dir`][custom_dir] setting:
...
!!! danger "Theme Extension Prerequisites"

    As the [`custom_dir`][custom_dir] setting is used for the theme extension process, Material for MkDocs needs to be installed via `pip`and referenced with the [`name [name] setting in `mkdocs.yml`. It will not work when cloning from `git`.

??? question "Structure in the overrides directory must mirror the directory structure of the original theme"
...
### Overriding Partials
!!! pied-piper "Overriding Partials"
...
!!! tip "Overriding Blocks"
    ...
    If you intend to __add__ something to a block rather than to replace it altogether with new content, use `{{ super() }}` inside the block to include the original block content. This is particularly useful when adding third-party scripts to your docs, e.g.
...
Next, create a new [Python virtual environment][venv] and [activate][venv-activate] it:
...
!!! warning "Ensure Pip Always Runs in a Virtual Environment"
...
Point your browser to [localhost:8000][live preview] and you should see this very documentation in front of you.
...
???+ tip "Building The Theme"
    ...
    1.  While this command will build all theme files, it will skip the overrides used in Material for MkDocs' own documentation which are not distributed with the theme. If you forked the theme and want to build the overrides as well, e.g. before submitting a PR with changes, use:
        ...
        1.1 This will take longer, as now the icon search index, schema files, as well as additional style sheet and JavaScript files are built.
