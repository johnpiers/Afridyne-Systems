...
!!! deep-dive "Logo Change"
    ...
    === ":octicons-package-16: Icon, bundled"
        ...
        1.  Enter a few keywords to find the perfect icon using our [icon
         search] and click on the shortcode to copy it to your clipboard:
...
!!! instruction "Header Logo Change"
...
!!! instruction "Change to a user-provided image"
...
| `admonition` | See [Admonition icons](../admonitions.md#admonition-icons)          |
...
!!! recommendation " Bootstrap Icons"

    You can now use all :fontawesome-brands-bootstrap: Bootstrap icons anywhere in Markdown files, as well as everywhere icons can be used in `mkdocs.yml`. However, note that the syntaxes are slightly different:

    - __Using icons in configuration__: take the path of the `*.svg` icon file starting at the `.icons` folder and drop the file extension, e.g. for `.icons/bootstrap/envelope-paper.svg`, use:
    ...
    - __Using icons in Markdown files__: additionally to taking the path from the `.icons` folder as noted above, replace all `/` with `-` and enclose the icon shortcode in two colons:
