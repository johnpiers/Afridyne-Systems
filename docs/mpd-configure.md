...
!!! desc "`mpd-configure`"
    ...
    - [audiophile-mpd](https://www.ap-linux.com/tag/mpd "What is MPD?")
    - [ALSA project - the C library reference](https://www.alsa-project.org/alsa-doc/alsa-lib/index.html "Index, Preamble and License")
...
!!! desc "`Storing Output of Script in a File`"
...
!!! desc "More Advanced Usage Example"
...
!!! desc "Fully Automated Usage Example"
    ...
    ```
    CONF_MPD_MUSICDIR="/srv/media/music" CONF_MPD_HOMEDIR="/var/lib/mpd" \ bash mpd-configure -l u -n -o "/etc/mpd.conf"
    ```
...
!!! desc "About the Alsa-capabilities Helper Script"
...
!!! desc "Background"
    Music Player Daemon (MPD) is a flexible, powerful, server-side application for playing music. Through plugins and libraries it can play a variety of sound files while being controlled by its network protocol. See the article. [How to turn Music Player Daemon (mpd) into an audiophile music player](https://readthedocs.io).

    It does this by creating a proper formatted `audio_output` configuration snippet for mpd's [alsa audio output plugin](http://musicpd.org) using the sound cards hardware address and turning all options off which might cause mpd to alter the incoming sound. For example:
...
!!! desc "Detailed Usage Instructions"
...
## [](https://gitlab.com/sonida/mpd-configure#usage-a-as-systemd-service)Usage as a Systemd Service

!!! desc "`Usage as a Systemd Service`"
...
!!! desc "`Usage from within another bash or sh script`"
...
!!! desc "`LTSP-specific Auto Logon Sample`"
...
!!! desc "General Environment Variables"
...
!!! deep-dive "Reference for both MPD/LTSP."
    MPD specific:
