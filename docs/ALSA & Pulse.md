...
<H4 style="text-align: center;">A complete audio guide for Linux, as well debunking the myths regarding ALSA and Pulse.</H4>

!!! desc "Linux Audio Users Guide"
...
### How and what can I hear?

!!! abstract "`Sound is a Wave that travels through Air`"
...
!!! ex "MYTH BUST"
    ...
    <u>Humans cannot hear beyond</u> <b>20 kHz</b>, </u>period</u>. <font color="#008B72">*__Anyone who claims to be able to is either supernatural or lying to you - I'll let you choose which.__*</font>
    
    To catch someone making false claims about hearing 16KHz - 20KHz+, administer an ABX double-blind test using a pure sine-tone or filtered noise. The subject must identify which of the three clips (A, B, or X) is the active high-frequency tone. To eliminate guesswork, require they correctly identify the frequency at least 8 to 10 times in a row.
    
    [Blind Testing a 16 kHz Upper Hearing Limit](https://www.audiocheck.net/blindtests_frequency.php)
...
!!! education " Bit-depth"
    ...
    !!! important "MYTH BUST"
        ...
        <font color="#00A99D">*__24-bit depth__ is often touted as 'high-resolution audio', claiming benefits of a better sonic experience. Such is nothing more than marketing speech, there is no meaningful data __24-bit__ can capture that __16-bit__ cannot.*</font>
...
!!! desc "What is Resampling?"
    ...
    !!! important "MYTH"
        ...
        That [Downsampling](https://en.wikipedia.org/wiki/Downsampling_(signal_processing)) results in a loss of quality! This is only true in a technical sense, or if you are downsampling to less than __48kHz, 16-bit__ PCM. When you downsample a __96kHz, 24-bit__ PCM stream to a __48kHz, 16-bit__ stream, no meaningful data is lost in the process; because the discarded data lies outside of the human ear's hearing range.
...
!!! recommendation "What is Encoding?"
    ...
    !!! important "MYTH"
        ...
        MYTH Lossless sounds better than lossy. This is blatantly untrue, lossless audio formats were created for perservation and archival reasons. When you encode a lossy file from a lossless source, and you make sure that it's a __48kHz, 16-bit PCM__ encoding, you will not lose any important information. What is enough depends on the quality of the encoder. For __OggVorbis,192kbps__ is sufficient, for __MP3, 256kbps__ should be preferred. __320kbps_ is excessive and the highest quality supported by MP3. In general, _256kbps__ does the trick, but with storage being abundant these days, you can play it safe and use __320kbps__ if it makes you feel better.
        
    !!! important "MYTH"
    
        MYTH that CBR is better than VBR. There is no reason not to use VBR at all, there is no point in writing __256Kbps__ of data if there is only silence or a constant tone. Let your encoder do what it does best!
        
    !!! important "FACT"
    
        FACT that encoding a lossy format to another lossy format will result in a loss of data! You will compress data that is already compressed, which is really bad. When encoding to a lossy format, always use a high quality recording in a lossless format as the source!
...
!!! desc "More Info"
    ...
    This article from the guys at [xiph.org](https://people.xiph.org/%7Exiphmont/demo/neil-young.html), (The people who brought you FLAC and Ogg Vorbis) who explain it better than I can: [xiph.org](https://people.xiph.org/%7Exiphmont/demo/neil-young.html#toc_more)
    
    An excellent set of papers of which I wasn't aware before beginning my own article. They tackle many of the same points I do in greater detail.
    
    [Coding High Quality Digital Audio](https://www.meridian-audio.info/public/coding2,P5b2520,P5d.pdf.pagespeed.ce.yeZvG6GuKe.pdf) by Bob Stuart of Meridian Audio is beautifully concise despite its greater length. Our conclusions differ somewhat (he takes as given the need for a slightly wider frequency range and bit depth without much justification), but the presentation is clear and easy to follow. [*Edit: I may not agree with many of Mr. Stuart's other articles, but I like this one a lot.*](https://people.xiph.org/%7Exiphmont/demo/neil-young.html#toc_more:~:text=%C2%A0%C2%A0%C2%A0%C2%A0%C2%A0%E2%80%94Carl%20Sagan-,Further%20reading,-Readers%20have%20alerted) 
    
    [Sampling Theory For Digital Audio](https://lavryengineering.com/pdfs/lavry-sampling-theory.pdf) by Dan Lavry of Lavry Engineering is another article that several readers pointed out. It expands my two pages or so about sampling, oversampling, and filtering into a more detailed 27 page treatment. Worry not, there are plenty of graphs, examples and references.
    

    
![Local image](./images/tldr.jpg){: style="display: block; margin: 0 auto"}

<H3 style="text-align: center;">TL;DR  I Just Want The Best Sound Quality!</H3>

!!! desc "Quick Guide"

    __Here is a quick guide to achieving great sound quality on Linux with the above in mind.__
    ...
    __As you can see, there's little you can do in Linux in the first place, so what can you do if you want better sound?__
    ...
    !!! important "MYTH"
        ...
        MYTH that Linux sound quality is worse than Windows. They are basically the same, Pulse doesn't function any differently as opposed to how MS Windows mixes and resamples.
        
    !!! important "MYTH"
    
        MYTH that Linux sound quality can be better than Windows. They are exactly the same. All improvements in quality come from the driver and your DAC, not the sound server. Pulse and ALSA do not touch the PCM beyond moving it around and resampling it.
...
!!! important "Attribution & Acknowledgments"
    This article was heavily inspired by and adapted from an excellent comprehensive Reddit guide originally titled *"A complete audio guide for Linux, as well debunking the myths regarding ALSA and Pulse."* 

    I have updated, corrected, and polished the original material to add more clarity. Unfortunately, when returning to link the original thread, the post had been taken down or removed, making it impossible to find the original author's handle. If you wrote the original guide and find this version, please reach out—I would love to properly credit you by name!


??? ex "__Linux Audio Stack and Architecture__"
    
    !!! recommendation "Notes And Additions"
        **24bit** is also useful for recording live inputs. You're less likely to get clipping. Once you have a good sample, sure, reduce it to 16. But for recording it is useful.

    ### Linux Audio Stack and Architecture

    <font color="#8DC73E">**PulseAudio**</font>
    ...
    <font color="#8DC73E">**How to output audio to a Bluetooth headset?**</font>
    ...
    <font color="#8DC73E">**How to output audio to TV/monitor via HDMI?**</font>
    PulseAudio also has syncs for various graphics cards if they are not supported by the default ALSA modules.

    <font color="#8DC73E">**How to send recorded audio to a separate system?**</font>
    ...
    ![PulseAudio Diagram](https://makersweb.net/files/attach/images/1015/723/019/2d5163c7791e78805301964f420822e9.png){: style="display: block; margin: 0 auto"} <H6 style="text-align: center;">*PulseAudio Diagram*</H6>

    <font color="#8DC73E">**Advanced Linux Sound Architecture**</font>
    ...
    <font color="#8DC73E">**The concept of a sound server**</font>
    ...
    Linux system…
    <font color="#8DC73E">**Knowing this is important because most Linux distributions use this hierarchy.**</font>

    ![Linux Audio Architecture Stack Layer Diagram](assets/pulse.png){: style="display: block; margin: 0 auto"}
