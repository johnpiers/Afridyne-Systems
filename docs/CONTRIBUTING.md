...
!!! git "Writing Code"
    ...
    - This project is written in [Rust](https://www.rust-lang.org/) and formatted using the nightly [`rustfmt`](https://github.com/rust-lang/rustfmt) version.
    ...
    - The dependencies are linted with [`cargo-deny`](https://github.com/EmbarkStudios/cargo-deny) and unused dependencies are detected using [`cargo-machete`](https://github.com/bnjbvr/cargo-machete).
    
    - License identifiers and copyright statements are checked using [`reuse`](https://git.fsfe.org/reuse/tool). Toml files are formatted via [`taplo-cli`](https://crates.io/crates/taplo-cli).
    
    ---
    
    - Various [`just`](https://github.com/casey/just) targets are used to run checks and tests. [`shellcheck`](https://www.shellcheck.net/) is used to check the just targets for correctness.
    - In order to review the snapshot changes in tests, you can use [`cargo-insta`](https://github.com/mitsuhiko/insta).
    
    - Code examples in READMEs is tested via [`tangler`](https://github.com/wiktor-k/tangler). Links in markdown files or doc blocks are tested via [`lychee`](https://github.com/lycheeverse/lychee).
...
!!! info " Adding Translations"
    ...
    1. Add or change keys only in the English `.ftl` files in `locales/` and commit them here. Weblate will automatically pick up new keys after a push.
    
    1. Do not edit non-English translations directly in the repository. Instead, update them via Weblate and a respective translation merge request will be created in GitLab.
...
!!! desc  "Code Coverage Reports"
    ...
    When calling `just create-coverage-report with-docs`, doc tests are considered as well. However, this is still an [unstable nightly-only feature](https://github.com/rust-lang/rust/issues/85658) and it always requires to generate coverage data for unit tests (`just test`) and doc tests (`just test-docs`) first.
...
!!! ex "Writing Commit Messages"
    ...
    1. Avoid using the crate name as the commit scope. (e.g. `feat(alpm-types)`) The changelog entries will be generated for the associated crate accordingly using [`release-plz`] and [`git-cliff`].
    ...
    Improve error handling by adding specific error codes and messages to make debugging easier and more informative. This update enhances parsing accuracy and provides more context for handling parsing errors.
